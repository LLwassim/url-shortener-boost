...
describe('UrlService', () => {
  ...
  describe('createShortUrl', () => {
    ...
    it('should return existing URL if already shortened', async () => {
      const existingUrl = {
        ...
        getShortUrl: jest.fn().mockReturnValue('http://localhost:3000/existing'),
      };
    ...
    it('should throw error for existing custom alias', async () => {
      ...
      await expect(service.createShortUrl(customAliasDto)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should check URL safety when scanning is enabled', async () => {
      ...
      await expect(service.createShortUrl(createUrlDto)).rejects.toThrow(BadRequestException);
      expect(mockVirusTotalService.checkUrl).toHaveBeenCalledWith('https://example.com/test');
    });
  });
  ...
  describe('findByCode', () => {
    ...
    it('should return URL from database when not in cache', async () => {
      ...
      expect(mockRepository.findOne).toHaveBeenCalledWith({ where: { code: 'abc123' } });
  ...
  describe('incrementHitCount', () => {
    it('should increment hit count and update cache', async () => {
      ...
      expect(mockRepository.increment).toHaveBeenCalledWith({ code: 'abc123' }, 'hitCount', 1);
      expect(mockRedisService.setex).toHaveBeenCalled();
    });
    ...
    it('should not throw on database error', async () => {
      ...
      // Should not throw
      await expect(service.incrementHitCount('abc123')).resolves.toBeUndefined();
    ...
    it('should return paginated URL list', async () => {
      const mockUrls = [
        ...
        },
        {
          ...
          createdAt: new Date(),
          isExpired: jest.fn().mockReturnValue(false),
          getShortUrl: jest.fn().mockReturnValue('http://localhost:3000/def456'),
        },
      ];
  ...
  describe('URL normalization', () => {
    it('should normalize URL by removing tracking parameters', async () => {
      const urlWithTracking = 'https://example.com/page?utm_source=test&utm_medium=email&param=keep';
      const createUrlDto: CreateUrlWithContextDto = {
        url: urlWithTracking,
        clientIp: '192.168.1.1',
        userAgent: 'Mozilla/5.0...',
