...
export class LoggerService implements NestLoggerService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.logger = pino({
      ...
      base: {
        ...
        service: this.configService.get<string>('APP_NAME', 'url-shortener-boost'),
   ...
   */
  logQuery(query: string, parameters: any[], duration: number, requestId?: string): void {
    this.logger.debug({
      requestId,
      query,
      parameters,
   ...
   * Log cache operation
   */
  logCache(operation: string, key: string, hit: boolean, duration: number, requestId?: string): void {
    this.logger.debug({
      requestId,
      operation,
      key,
   ...
   * Log security events
   */
  logSecurity(event: string, details: Record<string, any>, severity: 'low' | 'medium' | 'high' = 'medium'): void {
    this.logger.warn({
      event,
      severity,
      type: 'security',
